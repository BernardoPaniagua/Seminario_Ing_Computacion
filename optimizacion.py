...
class optimizacion:
    ...
    def comparacion(self,x):
        ...
        y=np.array([t_BLN,t_BLNM,t_BFGS])
        x=np.array([1,2,3])
        my_xticks = ['BLN', 'BLNM', 'BFGS']
...
#     fR = lambda x: (a-x[0])**2 + b*(x[1]-x[0]**2)**2 
#     x = np.array([0,0])
#     optR=optimizacion(fR,max_iters=500)
#     optR.comparacion(x)

# Problema cámaras

datos=np.loadtxt(open("crime_dataChico.txt", "rb"), delimiter=",", skiprows=1,usecols=range(3,5))
n=2
...
optC=optimizacion(fCrimen,max_iters=5,tol=1e-5)
r=optC.busqueda_lineal_newton(x,modificada=True)
